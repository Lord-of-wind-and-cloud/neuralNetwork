...
def transform_type(input_file, output_file):
    with open(output_file, "w") as text_file:
        with open(input_file) as f:
            lines = f.readlines()
            for line in lines:
                # spilt the line by the ',' to many segments
                columns = line.split(',')
                for raw_type in category:
                    flag = False
                    # if the line that contains the collected type
                    if raw_type == columns[-1].replace("\n", ""):
                        # concatenate attribute with ',' and numOfType
...
if __name__ == '__main__':
    transform_type("dataset/kddcup.data_10_percent.txt", "processedData/kddcup.data_10_percent.txt")
    transform_type("dataset/corrected.txt", "processedData/corrected.txt")

# -------------import data------------------
from pymongo import MongoClient

# 将有些变量转化int和float后再存储
def isfloat(value):
    try:
        float(value)
        return True
...
if __name__ == '__main__':
    ...
    # 为training_set表的type属性增加索引
    db.training_data.create_index("training_set.type")
    with open("processedData/kddcup.data_10_percent.txt") as f:
        lines = f.readlines()
        for line in lines:
            columns = line.split(',')
            dic = {}
    ...
    with open("processedData/corrected.txt") as f:
        lines = f.readlines()
        for line in lines:
            columns = line.split(',')
            dic = {}
