...
def getDataSet(number, X_data, Y_data):
    ...
    # 读取MLII导联的数据
    record = wfdb.rdrecord('dataset/' + number, channel_names=['MLII'])
    data = record.p_signal.flatten()
    rdata = denoise(data=data)
    # 获取心电数据记录中R波的位置和对应的标签
    annotation = wfdb.rdann('dataset/' + number, 'atr')
...
def main():
    ...
    # 训练与验证
    model.fit(X_train, Y_train, epochs=30, batch_size=128, validation_split=RATIO, callbacks=callbacks)  # validation_split 训练集所占比例
    # 预测
